...
>
> 模型准备结束时，还有两类防止过早收尾的机制。计划对账检查最新 TaskPlan 是否仍有未完成或状态不一致的项目，如果需要，会生成运行时指令让模型重新处理；完成门禁则结合任务修改、验证证据、后台任务和停止原因等信息判断是否需要再补一轮。它们不是永远强制开启，而是受运行模式和配置控制；没有 TaskPlan 时，计划对账自然没有计划可以核对，但其他完成检查仍可以独立存在。这里的目标不是让程序代替模型判断业务完全正确，而是阻止一些可以确定识别的“只说完成、没有闭环”。（**完成门禁具体则怎么工作的？怎么判断完成未完成**）
>
...
>
> 可观测性方面，项目会为每个用户 Turn 记录 Provider 调用与重试、工具调用与失败、重复调用、首次修改位置、验证情况、完成门禁和最终停止原因等遥测快照。这些**遥测事件（什么是遥测事件**也追加到 Session JSONL，但不会投影成普通模型消息，也不包含完整 Prompt、工具参数、工具输出或密钥。它能支持本地诊断和离线汇总，但还不是带分布式 span、统一查询、在线告警和可视化的完整 Trace 平台。
>
