...
>
> 在执行调度上，我把几个容易混淆的维度分开了。前台和后台描述的是主 Agent 要不要等待真实结果；串行和并行描述的是同一批工具之间如何安排；流式和非流式描述的是模型响应方式。同一轮多个只读且进入并行白名单的工具可以在线程池中并行执行，存在依赖、修改状态或不满足安全条件的调用会保持顺序。流式模式下，同步工具会被放到 AnyIO 工作线程执行，Agent 协程等待结果，但主线程上的界面刷新和取消处理仍可继续，这不等于工具自动进入后台。
>
...
>
> 文件修改还有独立的写前审查。权限预检为 ALLOW，不代表用户一定不看具体 Diff；在启用写前审查且不是 bypass 时，write、edit、apply_patch、delete 等支持预览的操作会先 dry-run，生成用户实际要批准的变化。用户应用审核后的变更时，系统再次检查文件状态与预览是否仍然一致；如果期间被其他程序修改，预览会失效，模型必须重新读取和生成修改。这个机制降低了“用户看到的内容”和“实际落盘内容”不一致的风险，但它不是操作系统级事务，也无法预演任意 Shell、网络或数据库副作用。
>
> Planning 模块用于把长任务拆成可观察的计划。模型通过 task_update 工具创建或更新 TaskPlan，常见操作是把当前步骤从 in_progress 更新为 completed。更新参数带 expected_revision：提交时先在 Session 锁内读取最新计划，再比较版本、校验新计划并追加 task_plan_updated 事件。因此它同时使用了锁保护提交临界区和 revision 检测陈旧更新。模型拿旧 revision 提交时会得到失败结果，再根据最新计划重新生成更新，而不是悄悄覆盖别人的修改。
>
> 模型准备结束时，还有两类防止过早收尾的机制。计划对账检查最新 TaskPlan 是否仍有未完成或状态不一致的项目，如果需要，会生成运行时指令让模型重新处理；完成门禁则结合任务修改、验证证据、后台任务和停止原因等信息判断是否需要再补一轮。它们不是永远强制开启，而是受运行模式和配置控制；没有 TaskPlan 时，计划对账自然没有计划可以核对，但其他完成检查仍可以独立存在。这里的目标不是让程序代替模型判断业务完全正确，而是阻止一些可以确定识别的“只说完成、没有闭环”。
>
> 多 Agent 采用受控委派，而不是让多个 Agent 任意聊天。父 Agent 通过 delegate 创建新的子 Session，写入父 Session ID、父任务 hash、角色和委托内容，再启动一套使用相同 Agent Loop 实现、但配置不同的子 Agent。researcher 侧重检索和理解，reviewer 侧重审查，tester 侧重运行验证，coder 才获得直接文件修改能力。子 Agent 看不到父 Session 的完整上下文，只获得明确委托内容和必要线索；它也不能继续调用 delegate，避免递归创建失控的 Agent 树。完成后，子 Agent 的总结和证据作为 delegate 工具结果返回父 Agent，由父 Agent 统一决策。
>
...
>
> Skill 则使用渐进式披露。启动时只扫描 Skill 名称、描述、来源和路径，把目录控制在预算内；模型判断某个 Skill 与任务相关时，再调用 load_skill，把完整 SKILL.md 作为工具结果送入后续上下文。Skill 引用的其他资料不会全部自动递归加载，里面的 Python 或 JavaScript 也不会因为被加载就自动执行；模型仍然需要通过正常工具明确读取资源或调用 Shell，而且运行环境必须真的具备对应解释器和依赖。当前项目能够发现和加载已经安装的 Skill，但没有实现“模型描述一个 Skill，系统自动从网上下载并安装”的完整安装代理。
>
> 可观测性方面，项目会为每个用户 Turn 记录 Provider 调用与重试、工具调用与失败、重复调用、首次修改位置、验证情况、完成门禁和最终停止原因等遥测快照。这些遥测事件也追加到 Session JSONL，但不会投影成普通模型消息，也不包含完整 Prompt、工具参数、工具输出或密钥。它能支持本地诊断和离线汇总，但还不是带分布式 span、统一查询、在线告警和可视化的完整 Trace 平台。
>
