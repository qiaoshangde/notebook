...
>
> 第二部分是工具运行时。模型只负责提出调用，程序负责动态筛选工具、校验参数、检查权限并结算结果。同一轮多个只读工具可以并行，有依赖或副作用的工具串行；长任务可以进入后台线程池，先返回启动回执，完成后再通过通知进入 Session。无论成功、失败、拒绝还是中断，每个 tool_call 都必须有唯一终态结果，保证下一次模型请求的工具协议合法。
>
> 第三部分是 Session、恢复和上下文。用户消息、模型回复、工具事务、Checkpoint 和 TaskPlan 都追加到 JSONL，通过事件重放构建当前会话。重启后，有终态的工具不会重跑；缺少结果的调用会被补成“中断且结果未知”，提醒模型先检查现场。完整历史和模型输入是分开的：接近窗口上限时，系统依次裁剪旧任务文本、压缩工具结果、生成归档占位，最后才创建 Checkpoint 摘要。还没有被模型消费的工具结果不能提前有损压缩，归档原文也可以按 ID 取回。
>
> 第四部分是安全和任务完成控制。工具调用会被转换成结构化权限请求，再结合长期授权、权限模式和操作范围决定允许、拒绝或询问用户；文件修改还支持写前预览和预览过期检查。TaskPlan 使用 revision 做并发校验，Agent 准备结束时还会进行计划对账和完成门禁，避免模型只口头说完成却没有形成验证证据。
>
> 第五部分是多 Agent。父 Agent 可以委托 researcher、reviewer、tester 和 coder，它们使用独立子 Session，并按角色获得不同工具。后台 coder 必须运行在 Git Worktree 中，修改不会直接进入父工作区；子 Agent 也不能继续递归委托。不过当前 delegate 还不能恢复一个已经结束的旧子 Session，后台任务本身也不能跨进程恢复。
>
> 最后，我增加了本地 Turn 遥测和 Harbor 评测适配，用 Aider Polyglot 与固定的 Terminal-Bench 任务观察任务成功、工具调用、重试和验证情况。这个项目想证明的是，Coding Agent 的效果不只取决于模型，还取决于 Harness 能否用确定性程序控制状态、安全、上下文和异常恢复；跨 Session 长期语义记忆等能力目前还没有实现，我会把这些边界明确说清楚。
...
### 1.3 请详细介绍 FirstCoder（不限时间）

面试表达：
> FirstCoder 是我从零实现的一套本地 Coding Agent Harness。这里的 Harness 可以理解为模型之外的工程运行时：模型负责理解任务、分析证据和决定下一步，Harness 负责给模型准备上下文、暴露受控工具、执行真实操作、保存状态，并在权限等待、网络失败、上下文超长或程序重启后继续维护一致性。所以这个项目的重点不是再做一个终端聊天界面，而是把一个 Coding Agent 真正运行起来所需要的控制系统补完整。
>
> 从整体架构看，最上层是 CLI 和 TUI，负责接收用户输入、显示流式内容、工具状态、权限请求和任务计划；中间是 Agent 应用层，负责一次用户 Turn 怎样开始、循环和结束；周围由 Provider、Tools、Session、Context、Permissions、Planning、Runtime、Delegation、MCP、Skills 和 Observability 等模块提供能力。底层才是模型 SDK、文件系统、Shell、Git、网络和线程池。这样的划分是为了把模型的不确定判断和程序的确定性控制分开，也让不同模块能够独立测试和替换。
>
> 启动时，程序会先读取全局和项目配置，解析当前工作目录、权限模式、模型目录、MCP 服务和 Skill 目录，再集中创建 Provider、Session 服务、工具注册表、权限管理器、上下文服务、后台任务管理器和 Agent Loop。模型并不是写死在循环里的，当前实现可以通过统一的内部协议适配 OpenAI 兼容服务和 Anthropic 服务。配置中会描述模型标识、上下文窗口、是否支持图片、是否支持流式与工具调用，以及温度、最大输出和 reasoning effort 等请求参数。
>
> 当用户提交一个任务时，系统先把用户消息追加到 Session，再判断它与当前任务是不是同一个任务。任务边界不是简单看有没有换一个问题，而是维护当前任务 hash、候选任务 hash 和稳定计数，避免模型对短句或补充说明频繁抖动。进入模型调用前，程序还会修补上次中断留下的未闭合工具事务，收集后台任务完成通知、运行期间追加的用户指引和其他需要本轮处理的状态，然后重新构建当前 Session 视图。也就是说，每次请求不是拿一个长期存在的 messages 数组直接发出，而是从持久化事实重新得到当前有效视图。
>
> 一次发送给模型的请求，前面是当前运行环境的系统指令，包括 Agent 身份、项目规则和已经加载的必要能力；然后是经过投影的会话历史；最后是本轮动态指令。工具定义不作为普通对话文本拼进去，而是通过模型 API 的 tools 字段单独发送，也会占用上下文预算。系统不会把注册表里的所有工具都无条件暴露给模型，而是根据主 Agent 或子 Agent 角色、当前运行模式、是否允许用户交互、是否已有 TaskPlan、MCP 工具是否在本轮激活等条件动态筛选。这样既减少工具 Schema 的固定 Token，也避免模型调用当前不允许执行的能力。
>
> Provider 层负责屏蔽模型服务差异。Agent 内部只认识统一的消息、工具调用、工具结果和流式事件，具体适配器再把它们转换成不同供应商的格式，并把响应、停止原因、用量和异常重新归一化。错误不会全部无脑重试：超时、限流、网络和服务端错误被视为可能恢复的瞬时错误，采用有限次数和退避等待；认证、配置和普通参数错误直接返回；上下文超长则必须先改变输入，通过更强的压缩重新构造请求后再尝试一次。流式请求连续失败时可以退回非流式请求，但失败尝试产生的内部流事件要丢弃，不能把两次响应拼在一起。
>
> Agent Loop 的核心是“模型提出操作，程序执行操作，再把真实结果反馈给模型”。模型可能直接给出文本，也可能返回一个或多个 tool call。只有当一条 assistant 消息完整结束后，系统才会保存其中的工具调用并进入执行；流式过程中收到一半工具名或一半 JSON 参数时绝不会提前执行。工具执行结束后，结果按照原来的 tool_call_id 写入 Session，下一次请求再交给模型判断。这个循环可以在一个用户 Turn 内执行很多次，所以一次 Turn 不等于一次模型调用，一个开发任务也不一定只包含一个 Turn。
>
> 工具系统不是简单的函数表。每个工具有名称、描述、参数 Schema、执行器，以及可选的权限声明和运行能力。模型请求前先决定哪些工具可见，模型返回后再检查工具是否真的存在、当前是否仍然可用、参数结构是否满足要求，以及 MCP 工具是否已经激活。参数通过不代表操作就安全，后面还要进入权限和执行调度。模型选错工具、编造字段或传错类型时，系统会把结构化失败结果写回原工具调用，让模型有机会修正，而不是让异常直接打断整个 Agent Loop。
>
> 在工具执行上，前台和后台描述的是主 Agent 要不要等待真实结果；串行和并行描述的是同一批工具之间如何安排；流式和非流式描述的是模型响应方式。同一轮多个只读且进入并行白名单的工具可以在线程池中并行执行，存在依赖、修改状态或不满足安全条件的调用会保持顺序。流式模式下，同步工具会被放到 AnyIO 工作线程执行，Agent 协程等待结果，但主线程上的界面刷新和取消处理仍可继续，这不等于工具自动进入后台。
>
> 真正的后台工具会在权限通过后提交给独立后台线程池。主 Agent 先收到“任务已启动”的占位结果，这个结果已经闭合了原 tool_call_id；真实执行完成后不能再伪装成同一调用的第二个工具结果，而是生成独立的后台通知。通知不会在线程结束的瞬间直接触发模型，因为那会产生并发修改同一 Session 的问题；它会在下一次请求准备阶段被收集，再作为运行时信息交给模型。后台任务支持查询和协作式取消，但线程已经运行用户代码后不能保证立即停止，任务状态也只保存在当前进程内，所以程序重启后不能恢复原 Future 和线程。
>
> Session 是整个系统的事实基础。我使用追加式 JSONL 记录用户消息、模型回复、工具调用与结果、权限事件、Checkpoint、任务计划、上下文压缩和回合遥测等事件。文件以追加为主，不把旧事件原地改掉；需要表示计划更新或工具结果被压缩时，再追加新的事件。读取时按顺序重放这些事件，分别得到 SessionView 和 SessionRuntimeState。前者面向当前业务视图，例如消息、Checkpoint、metadata 和最新 TaskPlan；后者保存不适合写进自然语言消息的控制事实，例如任务边界、已被模型消费的工具结果、自动压缩失败和最近压缩记录。
>
> 事件日志也承担恢复职责。进程重新启动后，程序不是恢复以前的线程和 Python 对象，而是重放日志，再创建新的运行时对象。已有终态结果的工具不会重新执行；如果日志中只有 tool call、没有结果，系统会补一个“执行被中断，真实结果未知”的工具结果，让消息协议重新闭合。之所以不自动重试，是因为操作可能已经成功产生外部副作用，只是结果还没有来得及落盘。待审批的合法工具事务则有单独的恢复路径，可以等待用户继续决定。这个方案保证的是会话事实可解释、模型协议可继续，不是对文件、Shell 和外部服务提供通用的 exactly-once 分布式事务。
>
> Context 模块解决的是“完整历史应该保留，但模型每次不可能看见全部历史”。请求准备时，会按照当前模型的上下文窗口、输出预留、系统提示词、工具 Schema、文本历史和图片估算动态计算预算。没有 Checkpoint 时使用当前有效消息；有 Checkpoint 时，用旧历史摘要加边界后的真实 tail。构造过程中还要保持 assistant tool call 和 tool result 的配对关系，不能从一个孤立工具结果开始，也不能留下调用没有结果的非法消息序列。
>
> 当上下文接近高水位时，系统按 L1 到 L4 逐层处理。L1 主要裁剪已经确认属于旧任务的普通对话；L2 按搜索结果、Git Diff、测试日志、JSON、HTML 或源码等内容类型压缩较大的工具结果；L3 把过时、重复或已经被新结果替代的工具输出归档，并在原位置保留包含 archive_id 的占位符；L4 才调用模型生成结构化 Checkpoint 摘要，用它承接旧历史。压缩前有一个关键保护：新工具结果必须先真正进入一次成功的 Provider 请求，被标记为已经消费，才能接受后续有损压缩。否则可能出现工具明明执行成功，但模型从来没看见结果，恢复后结果又被压掉的问题。
>
> **归档也不是向量数据库或自动 RAG。原文仍保存在当前 Session 的归档目录中，模型看到占位符后，需要主动调用归档读取工具，并根据 archive_id 和可选查询取回有限片段；取回结果会作为新的工具结果进入下一次请求。当前项目因此可以说具备可重放的会话记忆和分层上下文压缩，但不能说已经实现跨 Session 的用户画像、长期语义知识库或自动向量召回。这也是我在介绍记忆系统时会主动说明的能力边界。**
>
> 权限系统位于模型调用和真实副作用之间。工具如果声明了权限规则，系统会根据工具参数构造结构化 PermissionRequest，其中包含读取、写入、删除、Shell、Git、网络或 MCP 等 action，以及具体路径、命令或服务 Scope。它先匹配已经持久化的长期授权规则，再结合 standard、aggressive 或 bypass 模式计算 ALLOW、ASK 或 DENY。ASK 时由用户选择本次允许、拒绝，或者在工具允许的情况下创建 Allow always 规则。长期规则写入 permissions.json，不属于 Session JSONL；当前权限模式主要是运行时状态，重启后不会因为上一次是 bypass 就自动恢复成 bypass。
>
> 文件修改还有独立的写前审查。权限预检为 ALLOW，不代表用户一定不看具体 Diff；在启用写前审查且不是 bypass 时，write、edit、apply_patch、delete 等支持预览的操作会先 dry-run，生成用户实际要批准的变化。用户应用审核后的变更时，系统再次检查文件状态与预览是否仍然一致；如果期间被其他程序修改，预览会失效，模型必须重新读取和生成修改。这个机制降低了“用户看到的内容”和“实际落盘内容”不一致的风险，但它不是操作系统级事务，也无法预演任意 Shell、网络或数据库副作用。**（使用的类似于乐观锁，比较预览摘要）**
>
> Planning 模块用于把长任务拆成可观察的计划。模型通过 task_update 工具创建或更新 TaskPlan，常见操作是把当前步骤从 in_progress 更新为 completed。更新参数带 expected_revision：提交时先在 Session 锁内读取最新计划，再比较版本、校验新计划并追加 task_plan_updated 事件。因此它同时使用了锁保护提交临界区和 revision 检测陈旧更新。模型拿旧 revision 提交时会得到失败结果，再根据最新计划重新生成更新，而不是悄悄覆盖别人的修改。（**悲观锁和乐观锁**）
>
> 模型准备结束时，还有两类防止过早收尾的机制。计划对账检查最新 TaskPlan 是否仍有未完成或状态不一致的项目，如果需要，会生成运行时指令让模型重新处理；完成门禁则结合任务修改、验证证据、后台任务和停止原因等信息判断是否需要再补一轮。它们不是永远强制开启，而是受运行模式和配置控制；没有 TaskPlan 时，计划对账自然没有计划可以核对，但其他完成检查仍可以独立存在。这里的目标不是让程序代替模型判断业务完全正确，而是阻止一些可以确定识别的“只说完成、没有闭环”。（**完成门禁具体则怎么，工作的？怎么判断完成未完成**）
>
> 多 Agent 采用受控委派，而不是让多个 Agent 任意聊天。父 Agent 通过 delegate 创建新的子 Session，写入父 Session ID、父任务 hash、角色和委托内容，再启动一套使用相同 Agent Loop 实现、但配置不同的子 Agent。researcher 侧重检索和理解，reviewer 侧重审查，tester 侧重运行验证，coder 才获得直接文件修改能力。（coder有git的工作树）子 Agent 看不到父 Session 的完整上下文，只获得明确委托内容和必要线索；它也不能继续调用 delegate，避免递归创建失控的 Agent 树。完成后，子 Agent 的总结和证据作为 delegate 工具结果返回父 Agent，由父 Agent 统一决策。
>
> 前台子 Agent 与父 Agent 可以共享工作区，所以 Session 隔离不等于文件隔离。后台 coder 因为会独立修改代码，必须从 Git HEAD 创建单独 **Worktree 和分支**，工具和权限根目录也重新绑定到该 Worktree。它完成后返回 Worktree 路径、变更文件和 Diff，父 Agent 再决定是否采用，不会自动合并。这个设计避免后台修改直接污染父工作区，但也有代价：父工作区中未提交的修改不会自动进入新 Worktree，Worktree 也不能隔离网络、凭据和操作系统资源。当前 delegate 每次都会创建新子 Session，没有 resume_session_id 参数，所以 tester 发现问题后只能再委托一个新 coder，不能原地唤醒以前已经结束的 coder。
>
> MCP 和 Skill 解决的是两种不同的扩展问题。MCP 用来接入外部工具服务：系统连接本地 stdio 或远程服务，发现工具后先转换成内部统一 Tool。为了避免把大量外部 Schema 一次全部发给模型，模型先使用 MCP 工具搜索，在当前 Turn 激活少量候选，再按正常工具流程执行；真正调用时仍然经过 FirstCoder 的校验、权限和结果结算。Function Calling 负责模型怎样提出结构化调用，MCP 负责宿主怎样连接外部服务，它们处在同一条链路的不同层，并不是二选一。
>
> Skill 则使用渐进式披露。启动时只扫描 Skill 名称、描述、来源和路径，把目录控制在预算内；模型判断某个 Skill 与任务相关时，再调用 load_skill（这是一个工具），把完整 SKILL.md 作为工具结果送入后续上下文。Skill 引用的其他资料不会全部自动递归加载，里面的 Python 或 JavaScript 也不会因为被加载就自动执行；模型仍然需要通过正常工具明确读取资源或调用 Shell，而且运行环境必须真的具备对应解释器和依赖。当前项目能够发现和加载已经安装的 Skill，但没有实现“模型描述一个 Skill，系统自动从网上下载并安装”的完整安装代理。
>
> 可观测性方面，项目会为每个用户 Turn 记录 Provider 调用与重试、工具调用与失败、重复调用、首次修改位置、验证情况、完成门禁和最终停止原因等遥测快照。这些**遥测事件（什么是遥测时间**也追加到 Session JSONL，但不会投影成普通模型消息，也不包含完整 Prompt、工具参数、工具输出或密钥。它能支持本地诊断和离线汇总，但还不是带分布式 span、统一查询、在线告警和可视化的完整 Trace 平台。
>
> Benchmark 方面，项目目前维护的是 Harbor 集成。Harbor 负责任务集、隔离环境、Agent 运行、独立 verifier 和结果产物，FirstCoder 通过适配器在任务容器里运行。仓库接入了 Aider Polyglot 的反馈修复协议，也准备了 Terminal-Bench 2.0 的固定六题回归。历史一次 Aider Polyglot 运行共有 225 个任务，其中 221 个获得明确 reward，213 个最终通过；213 除以 221 是 96.38%，端到端 mean 是 94.67%。但这次运行允许真实测试失败后进行一次同 Session 修复，也包含基础设施恢复，所以我会把它表述成特定反馈协议和运行条件下的历史结果，不会说成纯单次生成成功率，更不会直接等同生产质量。
>
> 如果总结这个项目的核心设计，我认为有三个关键词。第一个是确定性边界：模型可以决定做什么，但工具是否存在、参数是否合法、权限是否允许、结果如何落盘和什么时候必须停止，由程序控制。第二个是事实与视图分离：JSONL 保存完整事实，SessionView、运行状态和模型上下文只是针对不同用途的投影。第三个是失败可解释：网络错误、上下文超长、权限等待、工具失败、取消、中断和后台完成都尽量转成明确状态，而不是混在一段自然语言里。
>
> 我也会主动说明当前局限。Session 恢复不能保证外部副作用 exactly once；后台线程和任务状态不能跨进程恢复；子 Agent 有日志但 delegate 没有恢复旧子 Session 的入口；Worktree 不是系统沙箱；权限规则不能证明模型提出的操作在业务上一定正确；上下文摘要仍可能损失信息；跨 Session 的长期语义记忆还没有实现；历史 Benchmark 也不能替代当前版本和真实业务任务的持续评测。这些边界不是为了否定项目，而是说明我能够区分“已经由代码保证的行为”和“仍然需要继续建设的能力”。

核对依据：[应用组装](E:/agent/后端agent开发资料/FirstCoder/firstcoder/app/composition/factory.py:85)、[Agent 运行入口](E:/agent/后端agent开发资料/FirstCoder/firstcoder/agent/application/runner.py:29)、[Turn 引擎](E:/agent/后端agent开发资料/FirstCoder/firstcoder/agent/turn/engine.py:98)、[工具执行服务](E:/agent/后端agent开发资料/FirstCoder/firstcoder/tools/application/execution/service.py:237)、[Session 重放](E:/agent/后端agent开发资料/FirstCoder/firstcoder/session/application/replay.py:12)、[上下文投影](E:/agent/后端agent开发资料/FirstCoder/firstcoder/context/domain/projection/builder.py:44)、[权限策略](E:/agent/后端agent开发资料/FirstCoder/firstcoder/permissions/domain/policy.py:63)、[后台任务](E:/agent/后端agent开发资料/FirstCoder/firstcoder/runtime/application/background.py:26)、[子 Agent](E:/agent/后端agent开发资料/FirstCoder/firstcoder/delegation/application/subagent.py:59)、[MCP 适配](E:/agent/后端agent开发资料/FirstCoder/firstcoder/mcp/infrastructure/tool_adapter.py:25)、[Skill 加载](E:/agent/后端agent开发资料/FirstCoder/firstcoder/tools/infrastructure/session/load_skill.py:15)、[回合遥测](E:/agent/后端agent开发资料/FirstCoder/firstcoder/observability/application/turn_telemetry.py:1)、[Harbor 集成](E:/agent/后端agent开发资料/FirstCoder/benchmark/harbor/README.md:19)。

可能的追问：

1. FirstCoder 最核心的技术难点是什么？

   答：我认为最难的不是让模型返回一个工具名，而是把模型、工具和可恢复状态组成一条始终合法的事务链。模型输出是不确定的，文件和命令又会产生真实副作用，中间还可能发生权限暂停、网络失败、用户取消或程序崩溃。系统必须明确记录“模型提出了什么、程序是否执行、结果是什么、模型是否已经看到结果”，否则恢复时很容易重复执行，或者给模型发送只有调用没有结果的非法历史。所以我把 Session 事件、tool_call 终态、权限恢复和 Provider 投影放在同一个一致性问题下设计，而不是分别做几个功能按钮。

2. 为什么选择追加式 JSONL，而不是直接保存一个最新 Session 快照？

   答：最新快照读取快，但只能告诉我现在是什么状态，很难解释状态如何形成，也不利于恢复中间事务。追加事件可以保留用户消息、工具调用、工具结果、权限决定、计划版本和压缩过程，重放时能够检查调用是否闭合、计划 revision 是否连续。它实现简单，也方便导出和调试。代价是日志会持续增长，当前完整重建成本也随事件数量增加；Checkpoint 只减少模型输入，不会减少 JSONL 重放成本。规模继续增大时，我会增加带事件偏移和投影版本的可校验快照、日志分段及保留策略，但仍保留事件日志作为事实来源。

3. 如何保证模型没有看见过的新工具结果不会被上下文压缩丢掉？

   答：工具执行成功和模型消费结果是两个不同状态。结果写入 Session 以后，只有它真正进入一次成功完成的 Provider 请求，系统才追加消费标记；请求超时、取消、流式消息未完成都不能算消费。L2、L3 和 L4 选择压缩候选时会保护还没有被消费的工具结果和对应事务，避免它第一次进入模型之前就被缩减或藏进摘要。这个机制保证的是投影生命周期，不代表模型看到结果后一定理解正确，后者还需要任务级评测。

4. 权限审批和写前审查为什么要分成两层？

   答：权限审批回答“这类操作能不能做”，写前审查回答“这一次具体会改成什么”。例如长期授权允许写某个项目目录，只能说明 Scope 匹配，并不意味着用户愿意接受每一份 Diff。写前审查会先 dry-run 生成具体变化，用户确认后再校验预览是否过期。两层分开以后，可以复用长期授权减少重复询问，同时对真实修改保持逐次可见。反过来，Shell、网络和数据库等操作很难可靠生成唯一 Diff，只能依靠权限、范围、超时和执行环境控制，不能强行套用文件预览机制。

5. TaskPlan、计划对账和完成门禁分别解决什么问题？

   答：TaskPlan 保存模型当前拆出的任务及状态，让长任务进度可以持久化和展示；revision 与锁防止两个基于旧版本的更新相互覆盖。计划对账发生在模型准备收尾时，它比较最新计划状态，发现仍有未完成项或需要同步的状态，就要求模型继续处理。完成门禁关注更广的执行证据，例如任务是否修改过、是否验证、是否还有后台任务和当前停止原因。三者都只能约束程序能够观察到的事实，不能替代业务验收；而且计划与门禁能力受运行模式和配置控制，不能说每个交互 Turn 都必然经过完全相同的门禁。

6. 后台执行和同一轮工具并行为什么不能合并成一个概念？

   答：并行批次仍属于当前模型响应：系统同时执行几个互不依赖的只读工具，等待整批结果，再发起下一次模型请求。后台执行则提前用占位结果闭合原 tool_call，让主 Agent 在真实任务仍运行时继续，结果以后通过通知返回。两者的生命周期、协议形式和取消语义不同。并行批次结束时结果已经齐全；后台任务可能跨越多个 Turn，但当前只能跨 Turn、不能跨进程。把两者混在一起，会导致模型把“已经提交”误认为“已经完成”，也容易错误复用原 tool_call_id。

7. 四种子 Agent 为什么共用同一套 Agent Loop，还需要不同角色？

   答：Agent Loop 解决的是共同执行协议：准备请求、调用模型、执行工具、记录结果和结束。researcher、reviewer、tester、coder 的差异主要在系统指令、工具白名单、预算、是否允许修改和运行环境，没有必要复制四套循环。共用实现可以保证工具事务、Session 和错误处理规则一致；角色配置则提供最小能力边界。它仍不是强安全沙箱，例如 tester 能运行验证命令，命令本身仍可能产生副作用，所以权限和环境限制不能因为角色名存在就省略。

8. 后台 coder 使用 Git Worktree 后，是否就实现了完整隔离？

   答：没有。Worktree 主要隔离仓库工作目录和 Git 索引，子 Agent 的文件修改不会直接覆盖父目录，也便于父 Agent 检查独立 Diff。它从 HEAD 建立，因此父目录未提交内容不会自动带过去。Worktree 共享 Git 对象库，而且不能隔离操作系统账号、网络、进程、环境变量和外部服务；Shell 也不会因为当前目录变了就自动失去访问其他路径的能力。如果任务包含不可信代码，还需要容器、虚拟机、网络策略和凭据最小化等系统级隔离。

9. MCP 工具和 Skill 为什么都采用渐进式披露？

   答：原因都是固定上下文有限，但处理对象不同。MCP 服务可能暴露很多工具 Schema，因此先搜索目录、只激活当前 Turn 需要的工具；Skill 目录可能包含很长的操作指南和资源，因此启动时只展示名称与描述，用到时再加载完整 SKILL.md。这样减少 Token 和工具选择干扰。不过渐进式披露增加了一次“先发现再使用”的决策，模型可能搜错关键词或忘记加载，所以需要评测发现率、误激活率和最终调用成功率。MCP 激活和 Skill 加载也都不等于权限批准或代码自动执行，后续仍走正常工具链路。

10. 如何证明 FirstCoder 的工程设计真的提高了 Agent 效果？

    答：不能只根据模块数量或一份历史高分证明。我会把验证分成三层：第一层是确定性契约测试，验证工具调用闭合、权限恢复、Checkpoint 边界、计划 revision 和流式一致性；第二层是固定任务与模型的 A/B Benchmark，观察任务通过、重试、工具失败、Token、时延和人工介入；第三层是真实项目中的受控任务，检查业务正确性、误修改和接管成本。Harbor 的历史 Aider Polyglot 结果能说明某次特定配置下整条链路可运行，但包含反馈修复和基础设施条件，不能直接推导生产稳定性。真正有说服力的是同版本、同任务、同预算的持续回归以及失败案例分析。
