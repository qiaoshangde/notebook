...
>
> Provider 层负责屏蔽模型服务差异。Agent 内部只认识统一的消息、工具调用、工具结果和流式事件，具体适配器再把它们转换成不同供应商的格式，并把响应、停止原因、用量和异常重新归一化。错误不会全部无脑重试：**超时、限流、网络和服务端错误**被视为可能恢复的瞬时错误，采用有限次数和退避等待；认证、配置和普通参数错误直接返回；上下文超长则必须先改变输入，通过更强的压缩重新构造请求后再尝试一次。流式请求连续失败时可以退回非流式请求，但失败尝试产生的内部流事件要丢弃，不能把两次响应拼在一起。
>
...
>
> **工具系统**不是简单的函数表。每个工具有名称、描述、参数 Schema、执行器，以及可选的权限声明和运行能力。模型请求前先决定哪些工具可见，模型返回后再检查工具是否真的存在、当前是否仍然可用、参数结构是否满足要求，以及 MCP 工具是否已经激活。参数通过不代表操作就安全，后面还要进入权限和执行调度。模型选错工具、编造字段或传错类型时，系统会把结构化失败结果写回原工具调用，让模型有机会修正，而不是让异常直接打断整个 Agent Loop。
>
> 在**工具执行**上，前台和后台描述的是主 Agent 要不要等待真实结果；串行和并行描述的是同一批工具之间如何安排；流式和非流式描述的是模型响应方式。同一轮多个只读且进入并行白名单的工具可以在线程池中并行执行，存在依赖、修改状态或不满足安全条件的调用会保持顺序。流式模式下，同步工具会被放到 AnyIO 工作线程执行，Agent 协程等待结果，但主线程上的界面刷新和取消处理仍可继续，这不等于工具自动进入后台。
>
> 真正的**后台工具**会在权限通过后提交给独立后台线程池。主 Agent 先收到“任务已启动”的占位结果，这个结果已经闭合了原 tool_call_id；真实执行完成后不能再伪装成同一调用的第二个工具结果，而是生成独立的后台通知。通知不会在线程结束的瞬间直接触发模型，因为那会产生并发修改同一 Session 的问题；它会在下一次请求准备阶段被收集，再作为运行时信息交给模型。后台任务支持查询和协作式取消，但线程已经运行用户代码后不能保证立即停止，任务状态也只保存在当前进程内，所以程序重启后不能恢复原 Future 和线程。
>
...
>
> **事件日志**也承担恢复职责。进程重新启动后，程序不是恢复以前的线程和 Python 对象，而是重放日志，再创建新的运行时对象。已有终态结果的工具不会重新执行；如果日志中只有 tool call、没有结果，系统会补一个“执行被中断，真实结果未知”的工具结果，让消息协议重新闭合。之所以不自动重试，是因为操作可能已经成功产生外部副作用，只是结果还没有来得及落盘。待审批的合法工具事务则有单独的恢复路径，可以等待用户继续决定。这个方案保证的是会话事实可解释、模型协议可继续，不是对文件、Shell 和外部服务提供通用的 exactly-once 分布式事务。
>
> **Context 模块**解决的是“完整历史应该保留，但模型每次不可能看见全部历史”。请求准备时，会按照当前模型的上下文窗口、输出预留、系统提示词、工具 Schema、文本历史和图片估算动态计算预算。没有 Checkpoint 时使用当前有效消息；有 Checkpoint 时，用旧历史摘要加边界后的真实 tail。构造过程中还要保持 assistant tool call 和 tool result 的配对关系，不能从一个孤立工具结果开始，也不能留下调用没有结果的非法消息序列。
>
> 当**上下文**接近高水位时，系统按 L1 到 L4 逐层处理。L1 主要裁剪已经确认属于旧任务的普通对话；L2 按搜索结果、Git Diff、测试日志、JSON、HTML 或源码等内容类型压缩较大的工具结果；L3 把过时、重复或已经被新结果替代的工具输出归档，并在原位置保留包含 archive_id 的占位符；L4 才调用模型生成结构化 Checkpoint 摘要，用它承接旧历史。压缩前有一个关键保护：新工具结果必须先真正进入一次成功的 Provider 请求，被标记为已经消费，才能接受后续有损压缩。否则可能出现工具明明执行成功，但模型从来没看见结果，恢复后结果又被压掉的问题。
>
> **归档**也不是向量数据库或自动 RAG。原文仍保存在当前 Session 的归档目录中，模型看到占位符后，需要主动调用归档读取工具，并根据 archive_id 和可选查询取回有限片段；取回结果会作为新的工具结果进入下一次请求。当前项目因此可以说具备可重放的会话记忆和分层上下文压缩，但不能说已经实现跨 Session 的用户画像、长期语义知识库或自动向量召回。这也是我在介绍记忆系统时会主动说明的能力边界。
>
> **权限系统**位于模型调用和真实副作用之间。工具如果声明了权限规则，系统会根据工具参数构造结构化 PermissionRequest，其中包含读取、写入、删除、Shell、Git、网络或 MCP 等 action，以及具体路径、命令或服务 Scope。它先匹配已经持久化的长期授权规则，再结合 standard、aggressive 或 bypass 模式计算 ALLOW、ASK 或 DENY。ASK 时由用户选择本次允许、拒绝，或者在工具允许的情况下创建 Allow always 规则。长期规则写入 permissions.json，不属于 Session JSONL；当前权限模式主要是运行时状态，重启后不会因为上一次是 bypass 就自动恢复成 bypass。
>
> 文件修改还有独立的**写前审查**。权限预检为 ALLOW，不代表用户一定不看具体 Diff；在启用写前审查且不是 bypass 时，write、edit、apply_patch、delete 等支持预览的操作会先 dry-run，生成用户实际要批准的变化。用户应用审核后的变更时，系统再次检查文件状态与预览是否仍然一致；如果期间被其他程序修改，预览会失效，模型必须重新读取和生成修改。这个机制降低了“用户看到的内容”和“实际落盘内容”不一致的风险，但它不是操作系统级事务，也无法预演任意 Shell、网络或数据库副作用。**（使用的类似于乐观锁，比较预览摘要）**
>
