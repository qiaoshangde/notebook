...
### memory具体存什么内容，如何工作
FirstCoder的“记忆召回”不是向量库自动搜索，而是三种机制：
最近消息
    → 直接进入 Provider 上下文
...
大型工具结果
    → 变成 archive placeholder
    → 模型需要时调用 retrieve_archive
...
> 第二次模型调用是由 Agent Loop主动触发的，不是由工具直接调用模型。第一次模型返回 `tool_calls` 后，系统先持久化这条 assistant消息，再执行这一批工具。只有工具执行已经完成、结果已经写入Session、没有等待用户授权、没有被取消，并且还没有达到工具轮次上限时，Agent Loop才会重新构建上下文，发起第二次模型调用。如果工具执行失败但已经生成结构化失败结果，第二次调用仍然会发生，让模型根据错误决定重试或换方案；如果工具需要权限确认，循环会先暂停，等用户处理并完成工具结算后再调用模型。
...
用户批准或拒绝后，系统恢复原工具事务，产生最终工具结果，然后才继续调用模型。
...
通常会先结算这一批工具，再统一构建下一次请求，并不是每完成一个工具就调用一次模型。

##### 5. 达到工具轮次上限

即使工具已经执行完成，如果递增后的轮次达到上限，系统也不会继续调用模型，而是生成工具轮次受限的终止响应。

一句话总结：

> 第二次模型调用发生在“第一轮工具事务已经完成结算，并且Agent Loop确认还允许继续运行”之后。

可能的追问：

1. 权限批准以后，原来的工具调用是怎样恢复的？
2. 多个并行工具中有一个需要权限确认时，其他工具怎么处理？
3. 工具执行后、第二次模型调用前程序崩溃，恢复时如何判断是否继续？
4. 工具失败为什么不能直接结束Agent Loop？
5. 任务边界变化为什么要在第二次调用前先压缩上下文？
6. 工具轮次和Provider调用次数有什么区别？
7. 第二次Provider调用失败时，工具结果会不会丢失或重复执行？
8. 如何保证同一个tool call只产生一个终态tool result？
